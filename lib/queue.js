...
function Queue(connection, name, options) {
    ...
    this.watchdog = new Watchdog(this, { interval: options.timeoutInterval, gracePeriod: options.timeoutGracePeriod });
    if(options.timeoutInterval === undefined || options.timeoutInterval) {
        this.watchdog.start();
    }
}
