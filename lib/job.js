...
Job.prototype.fail = function(error, callback) {
    if(this.data.retries > 0){
      ...
      if(typeof this.data.retryDelayMS === 'string'){
          //It's a serialized function
          this.data.delay = new Date(new Date().getTime() + eval('(' + this.data.retryDelayMS + ')')(this.data));
      } else {
          //It's the number of milliseconds
          this.data.delay = new Date(new Date().getTime() + this.data.retryDelayMS);
      }
